<template>
  <div class="relative w-full overflow-hidden">
    <section
      class="relative flex flex-col z-10 p-6 w-full h-full justify-center items-center min-h-[60dvh]"
    >
      <div
        class="w-full max-w-2xl px-6 h-full flex flex-col justify-center items-center gap-8"
      >
    ...
    >
      ...
      >
        <div
          ...
          :key="box.title"
          class="w-full max-w-xs rounded-lg border border-slate-200 bg-white p-6 shadow-sm hover:-translate-y-1 transition-transform duration-300"
        >
          <h3 class="text-base font-semibold text-slate-900">
            {{ box.title }}
          </h3>
          <p class="mt-3 text-sm text-slate-500">{{ box.description }}</p>
        </div>
      </div>
    </section>
  </div>
...
import BgCircle from "@/components/Cicle.vue";
